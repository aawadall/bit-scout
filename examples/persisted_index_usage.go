...
func demonstrateManualLoading() {
	...
	index := index.NewPersistedSimpleIndex()

	// Open database manually
	if err := index.OpenDatabase("./data/manual.db"); err != nil {
	...
	if isEmpty {
		fmt.Println("Database is empty, adding sample data...")

		// Add some data
		doc := models.Document{
			ID:     "manual_doc",
			Text:   "This document was added manually",
			Source: "/manual/path",
			Vector: []float64{0.5, 0.6},
			Meta:   map[string]string{"type": "manual"},
		}
		...
		fmt.Println("Database has data, loading...")

		// Load existing data
		if err := index.LoadAllFromDatabase(); err != nil {
			log.Error().Err(err).Msg("Failed to load from database")
			return
		}
	}
	...
	if err != nil {
		...
		fmt.Printf("Index now contains %d documents\n", count)
	}
}
